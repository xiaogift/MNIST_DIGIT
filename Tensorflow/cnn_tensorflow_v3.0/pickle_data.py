...
def load_data(validation=False):
    ...
    f.close()
    print('Loaded MNIST data:', len(raw_train[0]), 'for training and', len(raw_test[0]), 'for testing.')
    if not validation: return list(zip(raw_train[0], [vectorized_label(y) for y in raw_train[1]])), raw_test
    return list(zip(raw_train[0], [vectorized_label(y) for y in raw_train[1]])), raw_validation, raw_test
